...
def row_norm(mat):
    ...
    for i in range(0, len(m)):
        min_val = np.min(m[i])
        max_val = np.max(m[i])
        m[i] = (mat[i]-min_val)/(max_val-min_val) if max_val-min_val != 0 else mat[i]-min_val   # 后改的
    return m
...
def getTransMat1(disMat1, geneMat1, disGeneMat, tao):
    ...
    W = normalize(mat3, axis=1, norm='l1')
    # W = row_norm(mat3)
    return W
...
def pgWalk2 (pai, C, tao, varbFile, train_test_file, outPreFile, outIterFile):
    global geneIN, geneNI, disIN, disNI
    ...
    for test_dis, test_g_set in test_dis_dic.items():
        ...
        if test_dis not in train_dis_dic: continue # 后加的
        ...
        for temp in sorted_score:
            ...
            if gene_name not in train_g_set and gene_name in train_gene_set:
                ...
                if t_counter >= len(test_g_set) * 2 and t_counter >= 200: break
...
pre = '../data/004/'
def init_net():
    dis_ge_file = pre + 'input.txt'
    dis_dis_file = pre + 'srna.txt'
    ge_ge_file = pre + 'gene.txt'
    varb_file = pre + 'out_varb.txt'
    alpha = 20
    beta = 100
...
def pgWalkMain():
    print('pgWalkMain...')
    varb_file = pre + 'out_varb.txt'
    train_test_file = pre + 'input.txt'
    out_pre_file = pre + 'outPre.txt'
    out_iter_file =  pre + 'outIter.txt'
    # pai = 0.7
    # C = 1e-10
    # tao = 0.5
    pai = 0.9 # 重启概率
